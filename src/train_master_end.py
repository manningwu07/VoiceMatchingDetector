...
BATCH_SIZE = 64  
EPOCHS = 20       
LR = 1e-3 # SGD needs higher LR         
MOMENTUM = 0.5 # Start with 0.9 momentum, cut in half every 5 epochs/when it explodes
...
class ArcFace(tf.keras.layers.Layer):
    def __init__(self, n_classes=10, s=25.0, m=0.50, regularizer=None, **kwargs):
...
if __name__ == "__main__":
    ...
    # --- CRITICAL: DELETE OLD OPTIMIZER STATE ---
    # We switched optimizers, so we cannot trust the old state in the h5 file.
    # We load weights, but we ignore the previous optimizer.
    if os.path.exists(SAVE_PATH):
        print(f"♻️  Resuming weights from: {SAVE_PATH}")
        try:
            # Load weights by name to avoid optimizer mismatch
            model.load_weights(SAVE_PATH, by_name=True, skip_mismatch=True)
            print("✅ Weights loaded (Optimizer reset).")
